			...
			if(intval($itemObj->article->article_type_uid) == intval($pObj->conf['couponNormalType']) || intval($itemObj->article->article_type_uid) == intval($pObj->conf['couponRelatedType'])){	// so it�s not normalTYPE, payment or delivery
				...
				foreach ($couponArticles as $currentCouponArticleUid) {
					...
					foreach($sessionCoupons as $sc => $scObj){
							...
							// where is variable $isId initialized?
							...
							#debug(array($code,$row,$itemObj->quantity));
							if($row !== NULL AND (($basket->getarticletypesumgross(1) < intval($row['limit_start'])) OR (intval($itemObj->quantity) != 1))) {
								#debug(intval($row['uid']), 'UNSET');
								unset($basket->basket_items[$item]);
								unset($GLOBALS['TSFE']->fe_user->sesData['coupons'][intval($row['uid'])]);
...
//
//		which sense is in following code, alos the was uses a wrong varaibale for arraykey? Joerg Sprung
//	   if (!empty($pObj->basket->basket_items[$currentCouponArticleUid]->tx_commercecoupons_addedbycouponid)) {
//			$couponCode = $GLOBALS['TYPO3_DB']->exec_SELECTquery('code', 'tx_commercecoupons_coupons', 'article=' .$pObj->basket->basket_items[$currentCouponArticleUid]->tx_commercecoupons_addedbycouponid);
